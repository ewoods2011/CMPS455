...
//---------------------------------------------------------------------
// List::GetThreadAt
//		Retrieve an item at a certain ID (index)
//
// Returns:
//  Pointer to the item at index threadId.
//---------------------------------------------------------------------
Thread *
List::GetThreadAt(int threadId)
{
	...
	thread = (Thread*)current->item;
	while (thread->getId() != threadId) {
...
void
List::RemoveThread(int threadId)
{
	...
	thread = (Thread*)current->item;
	//While the next item's thread is not the thread we're looking for.
	while (thread->getId() != threadId) {
		prev = current;
		current = current->next;
		thread = (Thread*)current->item;
...
void
List::SeverTies(int threadId)
{
	printf("Trying to sever ties\n\n");
	ListElement *current = first;
	Thread *thread;
	...
	if (first == last)
		return;
	//While there is still a current thread that is next
	while (current->next != NULL)
	{
		if(thread->parent != NULL){
			if (thread->parent->getId() == threadId) 
			{
				thread->parent == NULL;
			}
		}
